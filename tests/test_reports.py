...
def test_spending_by_category_with_date(sample_transactions: DataFrame) -> None:
    ...
    assert all(
        pd.to_datetime(result["Дата операции"], dayfirst=True) <= pd.to_datetime("15.03.2025", dayfirst=True)
    ), "Даты не должны быть позже 15.03.2025"


# Тест 2: Проверка фильтрации по категории и использования текущей даты
def test_spending_by_category_without_date(sample_transactions: DataFrame) -> None:
    ...
    assert len(result) == 2, (
        f"Ожидается 2 записи по категории 'еда' за последние 3 месяца, текущая дата: " f"{current_date}"
    )


# Тест 3: Проверка с пустым датафреймом
def test_spending_with_empty_dataframe() -> None:
...
def test_spending_with_invalid_date_format(sample_transactions: DataFrame) -> None:
    with pytest.raises(ValueError):
        spending_by_category(sample_transactions, category="еда", date="invalid_date")


# Тест 5: Проверка, если категории нет в данных
def test_spending_with_category_not_found(sample_transactions: DataFrame) -> None:
